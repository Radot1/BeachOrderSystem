...
def log_order_to_csv(order_data):
    ...
    # CSV headers
    fieldnames = ["timestamp", "seat", "item_name", "price", "notes"]
...
def print_receipt():
    try:
        ...
        # Create receipt with seat number on right
        seat = order_data['seat']
        receipt_lines = [
            f"{'BEACH BAR ORDER'.ljust(32)}{seat}",
            "="*32 + " " + "="*len(seat),
        ...
        receipt_lines += [
            ...
            "",
            f"Notes: {order_data.get('notes', 'None')}",
            "="*32,
            "Thank you!"
        ]
        
        # ESC/POS commands for bold and large text
        ESC = b'\x1B'
        seat_header = (
            ESC + b'!\x38' +  # Set large and bold
            seat.encode('utf-8') +
            ESC + b'!\x00'    # Reset text style
        )
        
        # Combine all content
        receipt_content = "\n".join(receipt_lines).encode('utf-8')
        data = receipt_content + (LINE_FEED * 3) + seat_header + (LINE_FEED * 3) + CUT_PAPER
