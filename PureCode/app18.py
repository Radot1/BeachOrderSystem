...
def log_order_to_csv(order_data):
    ...
    # Calculate daily total if the file exists
    daily_total = 0
    if csv_file.exists():
        with open(csv_file, mode='r', newline='', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                # Skip the ORDER TOTAL rows we previously added
                if row['item_name'] != 'ORDER TOTAL' and row['price'] and not row['price'].startswith('DAILY TOTAL'):
                    try:
                        daily_total += float(row['price'])
                    except ValueError:
                        pass

    # Append the order total and daily total
    with open(csv_file, mode='a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
